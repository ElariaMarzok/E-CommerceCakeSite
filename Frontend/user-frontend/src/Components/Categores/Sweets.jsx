...
export default function Sweets() {
  // 1. تعريف الحالة (State) لتخزين البيانات وحالة التحميل
...
const API_URL = import.meta.env.VITE_API_URL;
  ...
  useEffect(() => {
    const fetchSweets = async () => {
      try {
        ...
        // 3. فلترة البيانات لعرض الكاتيجوري 'sweets' فقط
        const filteredSweets = data.filter(item => item.category === 'sweets');
        
        setSweetItems(filteredSweets);
        setLoading(false);
      } catch (error) {
  ...
  // واجهة التحميل (Loading State)
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto py-20 text-center">
        <div className="animate-pulse text-pink-500 font-bold text-xl">Loading Sweets... 🍬</div>
      </div>
    );
  }
    ...
    <div className="bg-white py-12 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Our Special Sweets</h1>
          <div className="h-1 w-20 bg-pink-400 mx-auto rounded-full"></div>
        </div>

        {/* التحقق إذا كانت القائمة فارغة */}
        {sweetItems.length === 0 && (
          <p className="text-center text-gray-400 py-10">No sweets available at the moment.</p>
        )}
