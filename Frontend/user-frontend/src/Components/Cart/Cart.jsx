...
export default function Cart() {
  ...
  // 2. دالة لتحديث الـ State عند الكتابة
  ...
  // 3. التحقق من صحة البيانات (هل الحقول فارغة؟)
  const isFormValid = 
  ...
  const handleCompleteOrder = async (e) => {
    e.preventDefault();
    // أول خطوة: التأكد من صحة البيانات
    if (!validateForm()) {
    ...
    const orderData = {
        ...
        items: cart.map(item => ({
            ...
            size: item.size || "Standard" // مهم جداً: الـ Schema يطلب size بشكل إجباري
        })),
    ...
    try {
        ...
        if (response.ok) {
            ...
            // هنا سيخبرك السيرفر بالضبط ما هو الحقل الناقص
    ...
    <div className="min-h-screen bg-pink-50/30 py-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-8 justify-center md:justify-start">
          ...
          <h1 className="text-3xl font-bold text-gray-800">{t('cart.title')} ({cart.length})</h1>
        </div>
              ...
              cart.map((item) => (
                <div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-pink-100 flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-4">
                    <img src={`${API_URL}${item.img}`} alt={item.name} className="w-20 h-20 rounded-lg object-cover bg-pink-50" />
                    <div>
  ...
  />
              ...
              <div className="pt-4 border-t border-gray-100 mt-6">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-2xl font-black text-pink-600">$80.00</span>
                </div>
                <div className="flex justify-between items-center mb-6">
