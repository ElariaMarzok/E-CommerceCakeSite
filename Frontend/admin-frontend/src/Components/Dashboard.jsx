  ...
  useEffect(() => {
   ...
   async  function getData (){
          ...
          try {
                  ...
                  if (response.ok) {
                      // to show the change immediately without refreshing page
                      setCakes(data);
                  }
              } catch (error) {
...
const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this sweet masterpiece? 🎂🗑️")) {
        try {
            const response = await fetch(`http://localhost:4000/cakes/${id}`, {
                method: 'DELETE',
            });
            if (response.ok) {
                // to show the change immediately without refreshing page
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl overflow-hidden border border-gray-100 shadow-sm shrink-0">
                        <img src={cake.images && cake.images.length > 0 ? BASE_URL + cake.images[0] : null} alt={cake.name} className="w-full h-full object-cover" />
                      </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      {cake.prices && Array.isArray(cake.prices) ? (
                        ...
                        // حماية للمنتجات القديمة (إذا كان السعر رقم واحد)
