...
import { Eye, CheckCircle, Clock, Package, Phone, MessageCircle, Calendar, Star } from 'lucide-react';
...
export default function Order() {
  ...
  const toggleStatus = (id) => {
    ...
    if (selectedOrder && selectedOrder._id === id) {
      setSelectedOrder(prev => ({ ...prev, status: prev.status === 'Pending' ? 'Completed' : 'Pending' }));
    }
  };

  // حساب الأكثر مبيعاً بناءً على حقل items و name
  ...
  if (loading) return <div className="p-8 text-center font-black text-pink-500 animate-pulse">Loading Cake Orders...</div>;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm flex items-center gap-5 border-l-4 border-l-pink-400">
            ...
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Top Selling</p>
              ...
              <tbody className="divide-y divide-gray-50">
                {sortedOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-pink-50/20 transition-colors">
                    ...
                    {/* التعديل هنا: استخدام order.customer.fullName */}
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => toggleStatus(order._id)} className={`p-2 rounded-xl transition-all ${order.status === 'Completed' ? 'text-green-500' : 'text-gray-300 hover:text-green-500'}`}>
                          <CheckCircle size={18} />
                        </button>
                      </div>
                    </td>
  ...
  <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
    <div className="bg-white rounded-[3rem] w-full max-w-2xl overflow-hidden shadow-2xl">
      ...
      <div className="p-8 max-h-[50vh] overflow-y-auto">
        ...
        <div className="mb-6 bg-gray-50 p-5 rounded-3xl border border-gray-100">
          <p className="font-black text-gray-800 text-sm mb-2 uppercase tracking-tighter">
            📍 Address: <span className="font-medium text-gray-600 normal-case">{selectedOrder.customer?.address}</span>
          </p>
          <p className="font-black text-gray-800 text-sm uppercase tracking-tighter">
            📝 Notes: <span className="font-medium text-gray-400 italic normal-case">{selectedOrder.customer?.notes || "No notes"}</span>
          </p>
        </div>
        ...
        <h3 className="font-black text-gray-800 mb-4 px-2">Items Ordered:</h3>
        
        <div className="space-y-4">
          {selectedOrder.items?.map((item, index) => (
            <div key={index} className="flex items-center gap-4 bg-white p-4 rounded-3xl border border-gray-100 shadow-sm transition-all hover:border-pink-200">
              {/* Product Icon/Image Placeholder */}
              <div className="w-14 h-14 bg-pink-50 rounded-2xl flex items-center justify-center text-pink-500 font-black text-[10px]">
                CAKE
              </div>
      ...
      <div className="p-10 bg-gray-50 border-t border-gray-100">
        ...
        <button 
          onClick={() => { toggleStatus(selectedOrder._id); setIsModalOpen(false); }}
        ...
        >
          {selectedOrder.status === 'Completed' ? 'Order Completed ✓' : 'Mark as Completed'}
        </button>
      </div>
    </div>
  </div>
